...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        if Config.validate_graph_memory() == []:
            try:
                self.zep_client = create_graph_memory_provider(api_key=zep_api_key)
            except Exception as e:
                logger.warning(f"图谱记忆 provider 初始化失败: {e}")
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        def search_edges():
            ...
            for attempt in range(max_retries):
                try:
                    return self.zep_client.search_graph(
                        graph_id=self.graph_id,
                        query=comprehensive_query,
                        options=GraphMemorySearchOptions(limit=30, scope="edges", reranker="rrf")
                    )
                except Exception as e:
                    last_exception = e
            ...
            for attempt in range(max_retries):
                try:
                    return self.zep_client.search_graph(
                        graph_id=self.graph_id,
                        query=comprehensive_query,
                        options=GraphMemorySearchOptions(limit=20, scope="nodes", reranker="rrf")
                    )
                except Exception as e:
                    last_exception = e
                    if attempt < max_retries - 1:
